...
const LineChartCompHistMora = () => {
  ...
  const [selectedDataKeyG1, setSelectedDataKeyG1] =
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          getApiUrl(`/principal/historico_morosidad?email=${queryParameter}`),
          options
        );
  ...
  useEffect(() => {
    const units = data[selectedDataKeyG1].map((item: any) =>
      parseFloat(item.tasa_morosidad)
    );
    if (units.length > 0) {
  ...
  const tranformeDataApi = (data: HistoricoMora, selectedDataKeyG1: string) => {
    return (
      data[selectedDataKeyG1 as keyof HistoricoMora] as TasaMorosidad[]
    ).map((item) => ({
      x: item.fecha,
...
export default LineChartCompHistMora;
