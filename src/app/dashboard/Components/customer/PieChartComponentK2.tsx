...
const PieChartComponentK2 = () => {
  ...
  const [selectedDataKeyK2, setSelectedDataKeyK2] =
    useState<string>("este_anho");
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          getApiUrl(`/clientes/porcentaje_ownership?email=${queryParameter}`),
          options
        );
          ...
          <ResponsivePie
            ...
            tooltip={(tooltipProps) => {
              ...
              const percentageValue = percentageKey
                ? responseData[selectedDataKeyK2][0][percentageKey] * 100
                : 0;

              // Creamos las etiquetas para el tooltip
              const percentageLabel = `${percentageValue.toFixed(0)}%`;
              const clientLabel =
                value > 1 ? `Clientes: ${value}` : `Cliente: ${value}`;
